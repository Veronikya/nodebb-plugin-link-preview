...
async function handleSpecialEmbed(url, $anchor) {
	...
	const { embedYoutube, embedVimeo, embedTiktok, embedBilibili } = await meta.settings.get('link-preview');
	...
	if (embedBilibili === 'on' && ['player.bilibili.com'].some(x => hostname === x)) {
		const video = url;
		const html = await app.renderAsync('partials/link-preview/bilibili', { video });
		$anchor.replaceWith(html);
